(ns gtfs.main
  "Kotodama WASM entrypoint for the Gtfs clean-room actor (L5) — Clojure port.
  ...
  facts (`gtfs.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Agency"   :plural "agencies"   :id-prefix "gtfs_age"
    :fields [:agencyId :agencyName :agencyUrl :agencyTimezone :agencyLang :agencyPhone :agencyEmail] :required [:agencyName :agencyUrl]
    :coerce {}                       :refs {}}
   {:entity "Route"    :plural "routes"    :id-prefix "gtfs_rou"
    :fields [:routeId :agencyId :routeShortName :routeLongName :routeDesc :routeType :routeColor :routeSortOrder :continuousPickup :continuousDropOff] :required [:routeShortName :routeLongName]
    :coerce {:routeType :int :routeSortOrder :int :continuousPickup :int :continuousDropOff :int} :refs {:agencyId "Agency"}}
   {:entity "Trip"     :plural "trips"     :id-prefix "gtfs_tri"
    :fields [:routeId :serviceId :tripId :tripHeadsign :directionId :blockId :shapeId :wheelchairAccessible :bikesAllowed] :required [:tripHeadsign :wheelchairAccessible]
    :coerce {:directionId :int :wheelchairAccessible :int :bikesAllowed :int} :refs {:routeId "Route" :tripId "Trip"}}
   {:entity "Stop"     :plural "stops"     :id-prefix "gtfs_sto"
    :fields [:stopId :stopCode :stopName :stopDesc :stopLat :stopLon :zoneId :locationType :parentStation :wheelchairBoarding] :required [:stopCode :stopName]
    :coerce {:stopLat :float :stopLon :float :locationType :int :wheelchairBoarding :int} :refs {:stopId "Stop"}}
   {:entity "StopTime" :plural "stoptimes" :id-prefix "gtfs_sto"
    :fields [:tripId :arrivalTime :departureTime :stopId :stopSequence :pickupType :dropOffType :timepoint :shapeDistTraveled] :required [:arrivalTime :departureTime]
    :coerce {:stopSequence :int :pickupType :int :dropOffType :int :timepoint :int :shapeDistTraveled :float} :refs {:tripId "Trip" :stopId "Stop"}}
   {:entity "Transfer" :plural "transfers" :id-prefix "gtfs_tra"
    :fields [:fromStopId :toStopId :transferType :minTransferTime] :required [:transferType :minTransferTime]
    :coerce {:transferType :int :minTransferTime :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"gtfs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gtfs-compat" :tier tier :entities entities} 200])
